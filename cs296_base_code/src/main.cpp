...
void average_time(){
	...
	int32 no_of_steps=0;
	float32 total_time=0.0000;
	float32 collision_time=0.0000;
	float32 velocity_update=0.0000;
	float32 position_update=0.0000;
	struct timeval begin, end;
	gettimeofday(&begin,NULL);
	double t1 = 1000*(begin.tv_sec) + begin.tv_usec/1000.0;
	while (no_of_steps<iterations){
	world->Step(step_size, settings.velocity_iterations, settings.position_iterations);
	total_time +=world_profile.step;
	collision_time +=world_profile.collide;
	velocity_update +=world_profile.solveVelocity;
	position_update +=world_profile.solvePosition;
	no_of_steps++;
	}
	...
	cout<<"Average time per step is "<<total_time/iterations<<" ms"<<endl;
	cout << "Average time for collisions is "<<collision_time/iterations<<" ms"<<endl;
	cout << "Average time for velocity updates is "<<velocity_update/iterations<<" ms"<<endl;
	cout << "Average time for position updates is "<<position_update/iterations<<" ms"<<endl<<endl;
	cout << "Total time for loop is "<< t<<" ms"<<endl;
}
